    ...
    def post(self, request, *args, **kwargs):
        ...
        if product_id =="":
            if Product.objects.filter(product_name=product_name,is_active=0).exists():
                ...
                r=Product(product_name=product_name,MTBF=MTBF,MTBSAF=MTBSAF,MTTR=MTTR,availability_target=availability_target)
                r.save()
                FindUser = UserProfile.objects.filter(user_id=user_ID)
                now = datetime.now()
                current_time = now.strftime("%H:%M:%S")
            ...
            if Product.objects.filter(product_name=product_name,is_active=0).exists():
                if Product.objects.filter(product_name=product_name, product_id=product_id,is_active=0).exists():
                    Product.objects.filter(product_id=product_id).update(MTBF=MTBF,MTBSAF=MTBSAF,MTTR=MTTR,availability_target=availability_target)
                    if meg !='':
                        FindUser = UserProfile.objects.filter(user_id=user_ID)
                        now = datetime.now()
                        current_time = now.strftime("%H:%M:%S")
                ...
                Product.objects.filter(product_id=product_id).update(product_name=product_name,MTBF=MTBF,MTBSAF=MTBSAF,MTTR=MTTR,availability_target=availability_target)
                if meg !='':
                    FindUser = UserProfile.objects.filter(user_id=user_ID)
                    now = datetime.now()
                    current_time = now.strftime("%H:%M:%S")
...
class Config(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        MTTR = units[0].MTTR
        id = units[0].id
        return render(request, self.template_name,{'MTBFMTBSAF':MTBFMTBSAF,'MTTR':MTTR,'id':id})
    ...
    def post(self, request, *args, **kwargs):
        ...
        PBSUnit.objects.filter(id=id).update(MTBFMTBSAF=MTBFMTBSAF,MTTR=MTTR)
        if meg != '':
            P_id = request.session['P_id']
            user_ID = request.session['user_ID']
            FindUser = UserProfile.objects.filter(user_id=user_ID)
